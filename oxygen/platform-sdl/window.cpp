...
Window::Window(const std::string &title, const PixelExtent &extent)
    : sdl_window_(kSdl.MakeWindow(title.c_str(), SDL_WINDOWPOS_CENTERED,
          SDL_WINDOWPOS_CENTERED, extent.width, extent.height, {})) {
  ASLOG_TO_LOGGER(platform_logger, info, "SDL3 Window[{}] created", Id());
}
...
Window::Window(const std::string &title, const PixelExtent &extent,
    ...
    : sdl_window_(kSdl.MakeWindow(title.c_str(), SDL_WINDOWPOS_CENTERED,
          SDL_WINDOWPOS_CENTERED, extent.width, extent.height, flags)) {
  ASLOG_TO_LOGGER(platform_logger, info, "SDL3 Window[{}] created", Id());
}
...
Window::~Window() {
  if (sdl_window_ != nullptr) {
    ASLOG_TO_LOGGER(platform_logger, info, "SDL3 Window[{}] destroyed", Id());
...
void Window::DoMaximize() {
  ...
  if (IsBorderLess()) {
    ...
    ASDEBUG_TO_LOGGER(platform_logger,
        "Display usable bounds x={} y={} w={} h={}", usable_area.x,
        usable_area.y, usable_area.w, usable_area.h);
    ASDEBUG_TO_LOGGER(
        platform_logger, "Window size {}", nostd::to_string(Size()));
    ASDEBUG_TO_LOGGER(
        platform_logger, "Window position {}", nostd::to_string(Position()));
  }
}
