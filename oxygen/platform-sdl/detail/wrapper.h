...
public:
  ...
  auto operator=(
      WrapperInterface &&other) noexcept -> WrapperInterface & = delete;
  ...
  virtual auto MakeWindow(const char *title, int pos_x, int pos_y, int width,
      int height, const Window::InitialFlags &flags) const -> SDL_Window * = 0;
  ...
  virtual auto GetWindowId(
      SDL_Window *window) const -> platform::WindowIdType = 0;
  virtual auto GetNativeWindow(
      SDL_Window *window) const -> NativeWindowInfo = 0;
  virtual void SetWindowFullScreen(
      SDL_Window *window, bool full_screen) const = 0;
  ...
  virtual void SetWindowAlwaysOnTop(
      SDL_Window *window, bool always_on_top) const = 0;
  ...
  virtual void SetWindowSize(
      SDL_Window *window, int width, int height) const = 0;
  virtual void GetWindowSize(
      SDL_Window *window, int *width, int *height) const = 0;
  virtual void GetWindowSizeInPixels(
      SDL_Window *window, int *width, int *height) const = 0;
  virtual void SetWindowMinimumSize(
      SDL_Window *window, int width, int height) const = 0;
  virtual void SetWindowMaximumSize(
      SDL_Window *window, int width, int height) const = 0;
  virtual void SetWindowResizable(SDL_Window *window, bool resizable) const = 0;

  virtual void SetWindowPosition(
      SDL_Window *window, int pos_x, int pos_y) const = 0;
  virtual void GetWindowPosition(
      SDL_Window *window, int *pos_x, int *pos_y) const = 0;

  virtual void SetWindowTitle(
      SDL_Window *window, const std::string &title) const = 0;
  ...
  [[nodiscard]] virtual auto GetDisplays(
      int *count) const -> SDL_DisplayID * = 0;
  [[nodiscard]] virtual auto GetPrimaryDisplay() const -> SDL_DisplayID = 0;
  [[nodiscard]] virtual auto GetDisplayName(
      SDL_DisplayID display_id) const -> const char * = 0;
  virtual void GetDisplayBounds(
      SDL_DisplayID display_id, SDL_Rect *rect) const = 0;
  virtual void GetDisplayUsableBounds(
      SDL_DisplayID display_id, SDL_Rect *rect) const = 0;
  ...
  [[nodiscard]] virtual auto GetKeyName(
      SDL_Keycode key) const -> std::string_view = 0;
  [[nodiscard]] virtual auto
  GetActiveKeyboardModifiers() const -> SDL_Keymod = 0;

  // ---------------------------------------------------------------------------

  [[nodiscard]] virtual auto
  GetRequiredVulkanExtensions() const -> std::vector<const char *> = 0;
};
...
public:
  ...
  auto MakeWindow(const char *title, int pos_x, int pos_y, int width,
      int height,
      const Window::InitialFlags &flags) const -> SDL_Window * override;
  ...
  void SetWindowFullScreen(
      SDL_Window *window, const bool full_screen) const override {
  ...
  void SetWindowAlwaysOnTop(
      SDL_Window *window, const bool always_on_top) const override {
  ...
  void SetWindowSize(
      SDL_Window *window, const int width, const int height) const override {
    SdlCheck(SDL_SetWindowSize(window, width, height));
  }

  void GetWindowSize(
      SDL_Window *window, int *width, int *height) const override {
    SdlCheck(SDL_GetWindowSize(window, width, height));
  }

  void GetWindowSizeInPixels(
      SDL_Window *window, int *width, int *height) const override {
  ...
  void SetWindowMaximumSize(
      SDL_Window *window, const int width, const int height) const override {
    SdlCheck(SDL_SetWindowMaximumSize(window, width, height));
  }

  void SetWindowResizable(
      SDL_Window *window, const bool resizable) const override {
    SdlCheck(SDL_SetWindowResizable(window, resizable));
  }

  void SetWindowPosition(
      SDL_Window *window, const int pos_x, const int pos_y) const override {
    SdlCheck(SDL_SetWindowPosition(window, pos_x, pos_y));
  }

  void GetWindowPosition(
      SDL_Window *window, int *pos_x, int *pos_y) const override {
    SdlCheck(SDL_GetWindowPosition(window, pos_x, pos_y));
  }

  void SetWindowTitle(
      SDL_Window *window, const std::string &title) const override {
  ...
  [[nodiscard]] auto GetDisplayName(
      const SDL_DisplayID display_id) const -> const char * override {
    const auto *display_name = SDL_GetDisplayName(display_id);
    SdlCheck(display_name != nullptr);
    return display_name;
  }

  void GetDisplayBounds(
      const SDL_DisplayID display_id, SDL_Rect *rect) const override {
    SdlCheck(SDL_GetDisplayBounds(display_id, rect));
  }

  void GetDisplayUsableBounds(
      const SDL_DisplayID display_id, SDL_Rect *rect) const override {
    SdlCheck(SDL_GetDisplayUsableBounds(display_id, rect));
  }

  [[nodiscard]] auto GetDisplayOrientation(
      const SDL_DisplayID display_id) const -> SDL_DisplayOrientation override {
  ...
  [[nodiscard]] auto
  GetRequiredVulkanExtensions() const -> std::vector<const char *> override {
    uint32_t count{0};
  ...
  [[nodiscard]] auto GetKeyName(
      SDL_Keycode key) const -> std::string_view override {
