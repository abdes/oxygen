...
auto FindInputMappingContextEntry(
  ...
  for (const auto &[priority, is_active, mapping_context] :
    ...
    if (is_active) {
      if (mapping_context->Update(update_context.delta_time)) {
        // Input is consumed
        ASDEBUG_TO_LOGGER(input_logger,
            "Stopping updates to mapping contexts (input consumed)");
        break;
      }
    }
  }
...
void InputSystem::AddAction(const std::shared_ptr<Action> &action) {
  if (std::ranges::find(actions_, action) != std::ranges::cend(actions_)) {
    ASLOG_TO_LOGGER(input_logger, warn, "Action [{}] has already been added",
        action->GetName());
    return;
  }
...
auto InputSystem::GetActionByName(
    std::string_view name) const -> std::shared_ptr<Action> {
...
void InputSystem::AddMappingContext(
  ...
  if (GetMappingContextByName(context->GetName())) {
    ASDEBUG_TO_LOGGER(input_logger,
        "Input mapping context with [{}] already exists", context->GetName());
    return;
  }
...
auto InputSystem::GetMappingContextByName(
...
void InputSystem::ActivateMappingContext(
  ...
  if (found == std::ranges::cend(mapping_contexts_)) {
    ASLOG_TO_LOGGER(input_logger, warn,
