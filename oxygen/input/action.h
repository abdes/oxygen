...
class Action {
public:
  Action(std::string name, ActionValueType value_type);

  [[nodiscard]] auto GetName() const {
    return name_;
  }

  [[nodiscard]] auto GetValueType() const {
    return value_type_;
  }

  [[nodiscard]] auto GetValue() const -> const auto & {
    return value_;
  }
  ...
  void SetConsumesInput(const bool consume) {
    consume_input_ = consume;
  }

  // -- Action events ----------------------------------------------------------

  [[nodiscard]] auto OnCanceled() -> auto & {
    return on_canceled_;
  }
  [[nodiscard]] auto OnCompleted() -> auto & {
    return on_completed_;
  }
  [[nodiscard]] auto OnOngoing() -> auto & {
    return on_ongoing_;
  }
  [[nodiscard]] auto OnStarted() -> auto & {
    return on_started_;
  }
  [[nodiscard]] auto OnTriggered() -> auto & {
    return on_triggered_;
  }
  ...
  [[nodiscard]] auto IsCompleted() const {
  ...
  [[nodiscard]] auto IsTriggered() const {
