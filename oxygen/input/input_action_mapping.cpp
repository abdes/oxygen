...
namespace {
auto &input_logger = // NOLINT(*-avoid-non-const-global-variables)
    oxygen::log::Registry::Instance().GetLogger("Oxygen.Input");
}
...
void InputActionMapping::StartEvaluation() {
  ...
  ASDEBUG_TO_LOGGER(input_logger, "action {} triggers evaluation started",
      action_->GetName());
...
void InputActionMapping::NotifyActionCanceled() {
  ASDEBUG_TO_LOGGER(input_logger, "action {} cancelled", action_->GetName());
  action_->OnCanceled()(*action_);
  CompleteEvaluation();
}
void InputActionMapping::NotifyActionTriggered() {
  ASLOG_TO_LOGGER(input_logger, debug, "===> action triggered : {}", action_->GetName());
  action_->OnTriggered()(*action_, action_value_);
  any_explicit_triggered_ = false;
  all_implicits_triggered_ = true;
}
void InputActionMapping::NotifyActionOngoing() {
  ASDEBUG_TO_LOGGER(input_logger, "action {} trigger evaluation ongoing",
      action_->GetName());
...
void InputActionMapping::CompleteEvaluation() {
  ASDEBUG_TO_LOGGER(input_logger, "action {} trigger evaluation completed",
      action_->GetName());
...
void InputActionMapping::HandleInput(const platform::InputEvent &event) {
  ...
  case platform::InputEventType::kKeyEvent: {
    ...
    action_value_.Update(
        k_event.GetButtonState() == platform::ButtonState::kPressed);
  } break;
  case platform::InputEventType::kMouseButtonEvent: {
    const auto &mb_event =
        dynamic_cast<const platform::MouseButtonEvent &>(event);
    action_value_.Update(
        mb_event.GetButtonState() == platform::ButtonState::kPressed);
  } break;
...
auto InputActionMapping::DoUpdate(Duration delta_time) -> bool {
  ...
  if ((!found_explicit_trigger_ || any_explicit_triggered_) &&
    ...
    if (input_consumed) {
      ASDEBUG_TO_LOGGER(input_logger, "Input was consumed by action: {}",
          action_->GetName());
    }
  }
