...
public:
  ...
  [[nodiscard]] auto IsOngoing() const {
    return state_ == State::kOngoing;
  }

  [[nodiscard]] virtual auto IsTriggered() const -> bool {
    return triggered_;
  }
...
protected:
  ...
  virtual auto DoUpdateState(
      const ActionValue &action_value, Duration delta_time) -> bool = 0;
  ...
  [[nodiscard]] auto GetPreviousState() const {
...
public:
  ...
  [[nodiscard]] auto IsOneShot() const {
    return one_shot_;
  }
  void OneShot(bool enable = true) {
    one_shot_ = enable;
  }
...
public:
  ...
  [[nodiscard]] auto TriggerOnStart() const {
    return trigger_on_start_;
  }
  void TriggerOnStart(bool enable = true) {
    trigger_on_start_ = enable;
  }

  [[nodiscard]] auto GetTriggerLimit() const {
    return trigger_limit_;
  }
...
public:
  ...
  [[nodiscard]] auto GetTapReleaseThreshold() const {
    return threshold_;
  }
...
public:
  ...
  void SetLinkedAction(std::shared_ptr<Action> action);
...
public:
  ...
  void AddComboStep(std::shared_ptr<Action> action,
      ActionStates completion_states = ActionStates::kTriggered,
      float time_to_complete_seconds = 0.5F);
  void RemoveComboStep(uint32_t index);
  void ClearComboSteps();
  [[nodiscard]] auto GetComboSteps() const {
    return combo_steps_;
  }

  void AddComboBreaker(std::shared_ptr<Action> action,
      ActionStates completion_states = ActionStates::kTriggered);
  void RemoveComboBreaker(uint32_t index);
  void ClearComboBreakers();
  [[nodiscard]] auto GetComboBreakers() const {
    return combo_breakers_;
  }
