...
void ActionTriggerCombo::AddComboStep(
    std::shared_ptr<Action> action, const ActionStates completion_states,
...
void ActionTriggerCombo::RemoveComboBreaker(
...
auto ActionTriggerCombo::DoUpdateState(
  ...
  if ((current_step.action->GetCurrentStates() &
          current_step.completion_states) != ActionStates::kNone) {
    current_step_index_++;
