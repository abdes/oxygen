...
public:
  ...
  explicit InputSystem(Platform &platform);
  ...
  [[nodiscard]] auto GetMappingContextByName(
      std::string_view name) const -> std::shared_ptr<InputMappingContext>;
  void ActivateMappingContext(
      const std::shared_ptr<InputMappingContext> &context);
  void DeactivateMappingContext(
      const std::shared_ptr<InputMappingContext> &context);

  void ProcessInput(const platform::InputEvent &event) override;
  void Update(const engine::SystemUpdateContext &update_context) override;
